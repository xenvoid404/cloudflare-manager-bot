...
class CloudflareAccountHandler:
    """Handler for managing Cloudflare account addition process."""
    
    def __init__(self):
        self.user_data: Dict[int, Dict[str, Any]] = {}
    
    async def start_add_account(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
        ...
        try:
            ...
            logger.error(f"Error starting add account for user {update.effective_user.id}: {e}")
            await update.callback_query.edit_message_text(
    ...
    async def handle_email(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
        ...
        try:
            ...
            logger.error(f"Error handling email for user {update.effective_user.id}: {e}")
            await update.message.reply_text(
    ...
    async def handle_api_key(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
        ...
        try:
            ...
            logger.error(f"Error handling API key for user {update.effective_user.id}: {e}")
            await update.message.reply_text(
                "⚠️ Terjadi kesalahan. Gunakan /menu untuk memulai ulang."
            )
            return ConversationHandler.END
    
    async def handle_account_id(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
        ...
        try:
            ...
            logger.error(f"Error handling account ID for user {update.effective_user.id}: {e}")
            await update.message.reply_text(
                "⚠️ Terjadi kesalahan. Gunakan /menu untuk memulai ulang."
            )
            self._cleanup_user_data(user.id)
    ...
    async def handle_zone_selection(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
        ...
        try:
            ...
            logger.error(f"Error handling zone selection for user {update.effective_user.id}: {e}")
            await update.callback_query.edit_message_text(
                "⚠️ Terjadi kesalahan. Gunakan /menu untuk mencoba lagi."
            )
    ...
    async def _fetch_zones(self, user_id: int) -> Optional[list]:
        ...
        try:
            if CloudFlare is None:
                logger.error("CloudFlare package not installed")
                return None
...
cf_handler = CloudflareAccountHandler()

# Create conversation handler
add_cloudflare_conversation = ConversationHandler(
    entry_points=[CallbackQueryHandler(cf_handler.start_add_account, pattern="^add_cf_account$")],
    states={
        EMAIL: [MessageHandler(filters.TEXT & ~filters.COMMAND, cf_handler.handle_email)],
        API_KEY: [MessageHandler(filters.TEXT & ~filters.COMMAND, cf_handler.handle_api_key)],
        ACCOUNT_ID: [MessageHandler(filters.TEXT & ~filters.COMMAND, cf_handler.handle_account_id)],
        ZONE_SELECTION: [CallbackQueryHandler(cf_handler.handle_zone_selection)],
    },
    fallbacks=[MessageHandler(filters.COMMAND, cf_handler.cancel)],
