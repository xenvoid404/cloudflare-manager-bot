...
class CloudflareService:
    ...
    async def get_dns_records(self, zone_id: str) -> List[Dict[str, Any]]:
        ...
        """
        try:
            response = self._make_request('GET', CloudflareAPI.dns_records_endpoint(zone_id))
            data = self._parse_response(response)
            ...
            logger.info(f"Successfully fetched {len(records)} DNS records for zone {zone_id}")
            return records
    ...
    async def create_dns_record(self, zone_id: str, record_data: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        try:
            response = self._make_request(
                'POST', 
                CloudflareAPI.dns_records_endpoint(zone_id),
                json=record_data
            )
            data = self._parse_response(response)
            
            record = data.get("result", {})
            logger.info(f"Successfully created DNS record {record.get('id')} in zone {zone_id}")
            return record
    ...
    async def update_dns_record(self, zone_id: str, record_id: str, record_data: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            logger.error(f"Error updating DNS record {record_id} in zone {zone_id}: {e}")
    ...
    async def delete_dns_record(self, zone_id: str, record_id: str) -> bool:
        ...
        try:
            ...
            logger.error(f"Error deleting DNS record {record_id} from zone {zone_id}: {e}")
    ...
    async def get_dns_record(self, zone_id: str, record_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            record = data.get("result", {})
            logger.info(f"Successfully fetched DNS record {record_id} from zone {zone_id}")
            return record
            
        except CloudflareAPIError:
            raise
        except Exception as e:
            logger.error(f"Error fetching DNS record {record_id} from zone {zone_id}: {e}")
